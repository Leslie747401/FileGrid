...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (session?.user) {
        setUser(session.user.user_metadata?.avatar_url);
      } else {
        setUser(null);
      }
      setIsLoading(false);
    ...
    <>
      <div className="w-full flex pt-8 sm:pt-12 pb-16 sm:pb-16 justify-between">
        ...
        <Link href={"/"}>
          <div className="flex gap-3.5 sm:gap-5 items-center">
            ...
            />
            <p className="text-[20px] sm:text-[30px] font-bold text-white flex justify-center">
              FileGrid
            </p>
          </div>
        </Link>
        ...
        <div className="flex gap-8 items-center">
          {isLoading ? (
            ...
            <>
              {/* Sign in Button  */}
              <button
                className="bg-white rounded-xl flex justify-center h-fit px-3 sm:px-4 py-2 gap-2"
                onClick={() => {
                  signInWithGoogle();
                  setIsLoading(true);
                }}
      ...
      {dropdown && (
        <div className="absolute top-[85px] right-[25px] sm:top-[105px] sm:right-16 bg-[#1e1e1e] border border-[#3e3e3e] rounded-xl">
          ...
          <button
            className="w-full rounded-xl flex justify-start pl-5 pr-16 py-2"
            onClick={() => {
              signout();
              setUser(null);
              setDropdown(false);
